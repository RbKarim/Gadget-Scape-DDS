...
IS
	...
	BEGIN
			...
			--> Selecting items from device 2 and also calculating the total price
			FOR i IN (SELECT * FROM Device2 WHERE device_name=selectedItem)LOOP
			...
			--> Selecting items from device 4 and also calculating the total price
			FOR j IN (SELECT * FROM Device4 WHERE device_name=selectedItem)LOOP
				DBMS_OUTPUT.PUT_LINE('Item Selected!!');
			...
			--> Selecting items from device 6 and also calculating the total price
			FOR k IN (SELECT * FROM Device6 WHERE device_name=selectedItem)LOOP
				DBMS_OUTPUT.PUT_LINE('Item Selected!!');
			...
			update customer1 set c_slot = 0 where c_id=cus_id;
